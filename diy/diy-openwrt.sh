...
configure_feeds() {
    ...
    # 🔧 修复 iwinfo/libiwinfo 依赖冲突（关键！）
    log_info "🔧 修复 iwinfo 依赖冲突..."
    
    # 1. 优先使用官方源中的 libiwinfo
    ./scripts/feeds install libiwinfo 2>/dev/null || true
    
    # 2. 如果 .config 中已启用 iwinfo，确保 libiwinfo 也被选中
    if grep -q "CONFIG_PACKAGE_iwinfo=y" .config 2>/dev/null; then
        # 强制启用官方 libiwinfo，禁用第三方版本
        sed -i 's/^# CONFIG_PACKAGE_libiwinfo is not set/CONFIG_PACKAGE_libiwinfo=y/' .config 2>/dev/null || true
        sed -i 's/^CONFIG_PACKAGE_libiwinfo=.*/CONFIG_PACKAGE_libiwinfo=y/' .config 2>/dev/null || true
    fi
    
    # 3. 清理可能的冲突包缓存
    rm -rf package/feeds/small/iwinfo package/feeds/small/libiwinfo 2>/dev/null || true
    
    log_info "✓ iwinfo 依赖修复完成"
}
...
# ============================================================================
# ⚔️ 模块 6: 冲突预检 (修复版 - 调用 run_conflict_resolver)
# ============================================================================
resolve_conflicts() {
    log_info "⚔️ 冲突预检..."
    [ ! -f ".config" ] && return 0
    
    # ✅ 修复：调用辅助脚本中的重命名函数，避免无限递归
    if declare -f run_conflict_resolver &>/dev/null; then
        run_conflict_resolver ".config"
    else
        # 内联兜底逻辑
